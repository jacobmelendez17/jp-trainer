...
export async function GET(req: Request) {
    ...
    if (!Number.isFinite(minLevel) || !Number.isFinite(maxLevel) || minLevel < 1 || maxLevel < minLevel) {
        return NextResponse.json({ error: "Invalid level range" });
    }
    ...
    if (!conn) return NextResponse.json({ error: "No WaniKani token saved" });

    const token = decryptToken(conn.tokenEnc);

    const assignments = await wkGetAll<Assignment>(
        token,
        `/assignments?subject_types=kanji&started=true&srs.stages=${GURU_PLUS_STAGES}`
    );
    ...
    for (const ids of idChunks) {
        ...
        if (!resp.ok) return NextResponse.json({ error: "WaniKani API error", details: json });
        subjects.push(...(json.data as Subject[]));
    }
